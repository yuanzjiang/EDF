...
from utils.utils_gsam import get_dataset, get_network, get_daparam, get_imagenet_1k, get_imagenet_1k_full, \
...
def train(args, channel, num_classes, im_size, trainloader, testloader, save_dir):
    ...
    k = 0
    while os.path.exists(os.path.join(save_dir, "replay_buffer_{}.pt".format(k))):
        k += 1

    if os.path.exists(os.path.join(save_dir, "checkpoint.pt")):
        trajectories = torch.load(os.path.join(save_dir, "checkpoint.pt"))

    total_its = args.num_experts - k * 10 - len(trajectories)
    print(f"Training {total_its} experts...")
    for it in range(total_its):
        ...
        torch.save(trajectories, os.path.join(save_dir, "checkpoint.pt"))
...
def main(args):
    ...
    if args.dataset != "ImageNet":
        channel, im_size, num_classes, class_names, mean, std, dst_train, dst_test, testloader, loader_train_dict, \
            class_map, class_map_inv = get_dataset(args.dataset, args.data_path, args.batch_real, args.subset,
                                                   args=args)

    else:
        if args.subset == 'full':
            channel, im_size, num_classes, class_names, mean, std, dst_train, dst_test, testloader, loader_train_dict, \
                class_map, class_map_inv = get_imagenet_1k_full(args, args.data_path)
        else:
            channel, im_size, num_classes, class_names, mean, std, dst_train, dst_test, testloader, loader_train_dict, \
    ...
    if args.subset == "full":
        if not os.path.exists("../imagenet_tensors"):
            images_all = []
            labels_all = []
            ...
            os.makedirs("../imagenet_tensors")
            torch.save(images_all, os.path.join("../imagenet_tensors", "images_all.pt"))
            torch.save(labels_all, os.path.join("../imagenet_tensors", "labels_all.pt"))
            torch.save(indices_class, os.path.join("../imagenet_tensors", "indices_class.pt"))
        else:
            print("Loading pre-saved ImageNet tensors...")
            images_all = torch.load(os.path.join("../imagenet_tensors", "images_all.pt"))
            labels_all = torch.load(os.path.join("../imagenet_tensors", "labels_all.pt"))
            indices_class = torch.load(os.path.join("../imagenet_tensors", "indices_class.pt"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--res', type=int, default=128, help='resolution of image')
