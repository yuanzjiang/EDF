...
1. **Evaluate complexity**: We first use our complexity metrics to evaluate all classes of ImageNet-1K. The complexity of an image if defined as the percentage of pixels with activation values above a predefined threshold (set to 0.5 in our case). The larger the value, the lower the complexity. Formally
   $$
   \frac{\sum_{h}\sum_{w} \mathbb{I}[M^{i}_{h,w} \geq 0.5]}{H \cdot W}
   $$
