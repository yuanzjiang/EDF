...
from utils.utils_baseline import get_imagenet_1k, get_dataset, get_network, get_eval_pool, evaluate_synset, get_time, \
    DiffAugment, plot_loss, ParamDiffAug
...
def create_soft_mask(activation_maps, num_classes, ipc, beta, threshold=0.5, mode='raw', descending=False):
    ...
    for i in range(num_classes * ipc):
        ...
        if threshold > 0.0:
            mask[activation_maps[i] >= threshold] = activation_maps[i][activation_maps[i] >= threshold] + beta
        else:
            mask[activation_maps[i] >= activation_mean] = activation_maps[i][activation_maps[i] >= activation_mean] + beta
...
def main(args):
    ...
    if args.dataset != "ImageNet":
        ...
        channel, im_size, num_classes, class_names, mean, std, dst_train, dst_test, testloader, loader_train_dict, \
            class_map, class_map_inv = get_imagenet_1k(args,
                                                       "/mnt/petrelfs/share/imagenet/images/",
                                                       "/mnt/hwfile/gveval/wangkai.p/imagenet/")
    ...
    for it in range(0, args.Iteration + 1):
        ...
        if it % args.interval == 0:
            activation_maps_deep = get_activation_maps(image_syn, label_syn, num_classes, args.ipc, im_size,
                                                       model_name=args.model_name,
                                                       target_layer=args.target_layer_deep,
                                                       model_path=args.activation_model_path,
                                                       dataset=args.dataset)

            torch.save(activation_maps_deep.cpu(),
                       os.path.join(save_dir, 'Normal', "activation_maps_deep_{}.pt".format(it)))
        ...
        softmask = create_soft_mask(activation_maps_deep, num_classes, args.ipc, beta=args.beta,
                                    threshold=args.act_threshold, descending=False)
