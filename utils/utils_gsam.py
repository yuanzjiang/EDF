...
from networks import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ResNet18, ResNet18BN_AP, ResNet18_AP, ResNet18BN, ResNet50
...
def get_imagenet_1k_full(args, data_path):
    class_map = None
...
def get_imagenet_1k(args, data_path):
    class_map = None
    loader_train_dict = None
    class_map_inv = None
    channel = 3
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        channel = 3
        # 使用命令行参数中的im_size（如果提供）
        if hasattr(args, 'im_size') and args.im_size > 0:
            im_size = (args.im_size, args.im_size)
            print(f"Tiny dataset: 使用命令行参数中的图像大小: {im_size}")
        else:
            im_size = (64, 64)
            
        # 使用命令行参数中的num_classes（如果提供）
        if hasattr(args, 'num_classes') and args.num_classes > 0:
            num_classes = args.num_classes
            print(f"Tiny dataset: 使用命令行参数中的类别数: {num_classes}")
        else:
            num_classes = 200
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug,scheduler, texture=False):
    ...
    if args.dataset == "ImageNet" and args.subset != 'full':
        class_map = {x: i for i, x in enumerate(config.img_net_classes)}
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if args.dataset == "ImageNet" and args.subset != 'full' and mode != "train":
